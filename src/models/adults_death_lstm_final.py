...
OUTPUT_DIR = ROOT_DIR / "outputs" / "adults_death_lstm_optimized"
...
class LSTMRegionTrainer:
    ...
    def plot_results(self, future_df, output_png):
        ...
        ax_plot.set_title(
            f"{self.region_name} | Adults 60+ Deaths | LSTM | "
        ...
        if not future_df.empty:
            ...
            ax_table.text(
                0.5, 1.02,
                "Future predictions of deaths in adults 60+ (per 100,000 inhabitants)",
    ...
    def plot_anomalies(self, future_df, output_png):
        ...
        ax.set_title(
            f"{self.region_name} | Epidemiological Anomalies in Deaths of Adults 60+",
...
def main():
    ...
    for region in REGIONS:
        ...
        results_df.to_csv(
            region_dir / f"{prefix}_adults_death_lstm_predictions.csv",
            index=False
        )

        metrics_df.to_csv(
            region_dir / f"{prefix}_adults_death_lstm_metrics.csv",
            index=False
        )

        future_csv_df.to_csv(
            region_dir / f"{prefix}_adults_death_lstm_future_predictions.csv",
            index=False
        )

        comparison_df.to_csv(
            region_dir / f"{prefix}_adults_death_comparison_predictions.csv",
            index=False
        )

        trainer.plot_results(
            future_df=future_df,
            output_png=region_dir / f"{prefix}_adults_death_lstm_plot.png"
        )

        trainer.plot_heatmap(
            future_df=future_df,
            output_png=region_dir / f"{prefix}_adults_death_heatmap.png"
        )

        trainer.plot_anomalies(
            future_df=future_df,
            output_png=region_dir / f"{prefix}_adults_death_anomalies.png"
        )
    ...
    final_results.to_csv(
        OUTPUT_DIR / "all_regions_adults_death_lstm_predictions.csv",
        index=False
    )

    final_metrics.to_csv(
        OUTPUT_DIR / "all_regions_adults_death_lstm_metrics.csv",
        index=False
    )

    final_future.to_csv(
        OUTPUT_DIR / "all_regions_adults_death_lstm_future_predictions.csv",
        index=False
    )

    final_comparisons.to_csv(
        OUTPUT_DIR / "all_regions_comparison_death_predictions.csv",
        index=False
    )
