...
OUTPUT_DIR = ROOT_DIR / "outputs" / "children_hosp_baseline_stat_optimized"
...
class BaselineStatTrainer:
    ...
    def plot_results(self, future_df, output_png):
        ...
        ax_plot.set_title(
            f"{self.region_name} | Children 5- Hospitalizations | Statistical Models | "
...
def main():
    ...
    for region in REGIONS:
        ...
        results_df.to_csv(
            region_dir / f"{prefix}_children_hosp_baseline_stat_predictions.csv",
            index=False
        )

        metrics_df.to_csv(
            region_dir / f"{prefix}_children_hosp_baseline_stat_metrics.csv",
            index=False
        )

        future_csv_df.to_csv(
            region_dir / f"{prefix}_children_hosp_baseline_stat_future_predictions.csv",
            index=False
        )

        comparison_df.to_csv(
            region_dir / f"{prefix}_children_hosp_baseline_stat_comparison_predictions.csv",
            index=False
        )

        trainer.plot_results(
            future_df=future_df,
            output_png=region_dir / f"{prefix}_children_hosp_baseline_stat_plot.png"
        )
    ...
    pd.concat(all_results, ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_baseline_stat_predictions.csv",
        index=False
    )

    pd.concat(all_metrics, ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_baseline_stat_metrics.csv",
        index=False
    )

    pd.concat(all_future, ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_baseline_stat_future_predictions.csv",
        index=False
    )

    pd.concat(all_comparisons, ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_baseline_stat_comparison_predictions.csv",
        index=False
    )
