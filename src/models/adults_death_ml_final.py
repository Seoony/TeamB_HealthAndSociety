...
class MLRegionTrainer:
    ...
    def plot_results(self, future_df, output_png):
        ...
        ax_plot.set_title(
            f"{self.region_name} | Adults 60+ Deaths | Optimized ML Models | "
    ...
    def plot_anomalies(self, future_df, output_png):
        ...
        ax.set_title(
            f"{self.region_name} | "
            "Epidemiological Anomalies in Deaths "
            "of Adults 60+",
            fontsize=14,
            fontweight="bold"
        )
...
def main():
    ...
    for region in REGIONS:
        ...
        results_df.to_csv(
            OUTPUT_DIR / f"{prefix}_adults_death_ml_predictions.csv", index=False)
        metrics_df.to_csv(
            OUTPUT_DIR / f"{prefix}_adults_death_ml_metrics.csv", index=False)
        future_df.to_csv(
            OUTPUT_DIR / f"{prefix}_adults_death_ml_future_predictions.csv", index=False)
        comparison_df.to_csv(
            OUTPUT_DIR / f"{prefix}_adults_death_ml_comparison_predictions.csv", index=False)
        trainer.plot_results(
            future_df=future_df,
            output_png=OUTPUT_DIR / f"{prefix}_adults_death_ml_plot.png"
        )
        trainer.plot_heatmap(
            future_df=future_df,
            output_png=OUTPUT_DIR / f"{prefix}_adults_death_ml_heatmap.png"
        )
        trainer.plot_anomalies(
            future_df=future_df,
            output_png=OUTPUT_DIR / f"{prefix}_adults_death_ml_anomalies.png"
        )

    pd.concat(all_results,     ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_adults_death_ml_predictions.csv",        index=False)
    pd.concat(all_metrics,     ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_adults_death_ml_metrics.csv",            index=False)
    ...
    pd.concat(all_comparisons, ignore_index=True).to_csv(
        OUTPUT_DIR / "all_regions_adults_death_ml_comparison_predictions.csv",            index=False)
