...
OUTPUT_DIR = ROOT_DIR / "outputs" / "children_hosp_lstm_optimized"
...
TARGET_COL = "hospitalizados_men5"
...
class LSTMRegionTrainer:
    ...
    def plot_results(self, future_df, output_png):
        ...
        ax_plot.set_title(
            f"{self.region_name} | Children 5- Hospitalizations | LSTM | "
    ...
    def plot_anomalies(self, future_df, output_png):
        ...
        ax.set_title(
            f"{self.region_name} | Epidemiological Anomalies in Hospitalizations of Children 5-",
...
def main():
    ...
    for region in REGIONS:
        ...
        results_df.to_csv(
            region_dir / f"{prefix}_children_hosp_lstm_predictions.csv",
            index=False
        )

        metrics_df.to_csv(
            region_dir / f"{prefix}_children_hosp_lstm_metrics.csv",
            index=False
        )

        future_csv_df.to_csv(
            region_dir / f"{prefix}_children_hosp_lstm_future_predictions.csv",
            index=False
        )

        comparison_df.to_csv(
            region_dir / f"{prefix}_children_hosp_comparison_predictions.csv",
            index=False
        )

        trainer.plot_results(
            future_df=future_df,
            output_png=region_dir / f"{prefix}_children_hosp_lstm_plot.png"
        )
    ...
    final_results.to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_lstm_predictions.csv",
        index=False
    )

    final_metrics.to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_lstm_metrics.csv",
        index=False
    )

    final_future.to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_lstm_future_predictions.csv",
        index=False
    )

    final_comparisons.to_csv(
        OUTPUT_DIR / "all_regions_children_hosp_comparison_predictions.csv",
        index=False
    )
